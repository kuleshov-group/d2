...
def evaluate(
    ...
    for batch in tqdm(dataloader, disable=(dist.get_rank() != 0)):
        ...
        completion_ids = out[:, -entropy.shape[1]:]
        is_eos = completion_ids == tokenizer.eos_token_id
        eos_idx = torch.full((is_eos.size(0),), is_eos.size(1), dtype=torch.long, device=device)
        eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(dim=1)]
        sequence_indices = torch.arange(is_eos.size(1), device=device).expand(is_eos.size(0), -1)
        completion_mask = (sequence_indices <= eos_idx.unsqueeze(1)).int()
        policy_entropy = (entropy * completion_mask).sum(-1) / completion_mask.sum(-1)
...
if __name__ == "__main__":
    ...
    filename = f"{args.output_dir}/{args.dataset}_{model_name}_{args.gen_length}_{args.diffusion_steps}_{args.block_length}_{dist.get_rank()}_generations.json"
