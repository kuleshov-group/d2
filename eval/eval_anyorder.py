...
from generate import generate_anyorder
...
def evaluate(
    ...
    ao_model.eval()
    total_processed = torch.tensor(0, device=ao_model.device)
    wall_times = []
    all_generations = []
    device = ao_model.device
    ...
    for batch in tqdm(dataloader, disable=(dist.get_rank() != 0)):
        ...
        # generating 2 tokens at a time is hard coded
        out = generate_anyorder(
            ao_model,
            input_ids,
            tokenizer,
            steps=steps,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, choices=["gsm8k", "math"], default="gsm8k"
    )
    ...
    ao_model = custom_llada.LLaDAModelLM.from_pretrained(args.model_path, trust_remote_code=True)
    ao_model = ao_model.to(torch.bfloat16).to(local_rank)

    tokenizer = AutoTokenizer.from_pretrained("GSAI-ML/LLaDA-8B-Base", trust_remote_code=True)

    if args.checkpoint_path:
        ao_model = PeftModel.from_pretrained(ao_model, args.checkpoint_path, torch_dtype=torch.bfloat16).to(
            local_rank
        )
    ...
    filename = f"{args.output_dir}/{args.dataset}_{model_name}_{args.gen_length}_{args.diffusion_steps}_{dist.get_rank()}_anyorder_generations.json"
