...
from SFT_AO.configuration_llada import (
  LLaDAConfig,
  StrEnum,
  InitFnType,
  ActivationType,
...
def ensure_finite_(
  x: torch.Tensor, check_neg_inf: bool = True, check_pos_inf: bool = False):
    ...
    dtype when ``check_neg_inf``
    is ``True`` and to replace ``float("inf")`` with the maximum value of the dtype when
    ``check_pos_inf`` is ``True``.
    """
    if check_neg_inf:
...
class BufferCache(dict, MutableMapping[str, torch.Tensor]):
    """
    Cache for attention biases and other things that would normally be stored as
    buffers.
    We avoid using buffers because we've run into various issues doing so with FSDP.
    In general, it appears the way FSDP handles buffers is not well-defined.
    It doesn't shard them, but apparently it does synchronize them across processes,
    which we want to avoid since (A) it isn't necessary, and (B) we sometimes have
    `-inf` in these biases which might get turned into NaNs when they're synchronized
    due to casting or some other issue.
    """
...
class RotaryEmbedding(nn.Module):
    ...
    def get_rotary_embedding(
      ...
      position_ids: Optional[torch.LongTensor] = None,
        ...
        def _select_positions(pos: torch.Tensor, position_ids: torch.Tensor) -> torch.Tensor:
            pos = pos.squeeze(0).squeeze(0)
            if position_ids.dim() == 1:
                return pos[position_ids][None, None, :, :]
            if position_ids.dim() == 2:
                return pos[position_ids][:, None, :, :]
        ...
        if (
            ...
            if position_ids is not None:
                ...
                return _select_positions(pos_sin, position_ids), _select_positions(pos_cos, position_ids)
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            if position_ids is not None:
                ...
                seq = torch.arange(max_pos, device=device, dtype=torch.float)
            else:
    ...
    def forward(
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            # ipdb.set_trace()
            pos_sin, pos_cos = self.get_rotary_embedding(
                key_len, q_.device, position_ids=position_ids)
...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        optional attention mask if passed, and applying dropout if a probability greater
        than 0.0 is specified.
        """
...
class LLaDASequentialBlock(LLaDABlock):
    ...
    `MLP(LN(x + Attention(LN(x))))`` (plus another skip connection).
    """
    ...
    def reset_parameters(self):
        ...
        init_weights(
          self.config, self.att_proj,
          d=self.config.d_model, layer_id=None, type_of_module=ModuleType.in_module
        )
        init_weights(
          self.config, self.ff_proj,
          d=self.config.d_model, layer_id=None, type_of_module=ModuleType.in_module
        )
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache = self._activation_checkpoint_fn(  # type: ignore
              ...
              layer_past=layer_past, use_cache=use_cache, position_ids=position_ids
            )
        else:
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.q_proj = nn.Linear(
          config.d_model, q_proj_out_dim,
          bias=config.include_bias | config.include_qkv_bias,
          device=config.init_device
        )
        self.k_proj = nn.Linear(
          config.d_model, k_proj_out_dim,
          bias=config.include_bias | config.include_qkv_bias,
          device=config.init_device
        )
        self.v_proj = nn.Linear(
          config.d_model, v_proj_out_dim,
          bias=config.include_bias | config.include_qkv_bias,
          device=config.init_device
        )
...
class LLaDAOutput(NamedTuple):
    ...
    """
    A tensor of shape `(batch_size, seq_len, vocab_size)` representing the log
    probabilities for the next token *before* normalization via (log) softmax.
    """
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            # NOTE (epwalsh): we need to initialize the attn bias in order for attn to
            # work properly with key+value cache.
            # Otherwise, `F.scaled_dot_product_attention()` doesn't seem to compute
            # scores correctly.
