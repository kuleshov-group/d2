...
class diffuGRPOTrainer(GRPOTrainer):
    ...
    def token_entropy_from_logits(self, logits, T):
        scaled = logits / T
    ...
    def generate(
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            entropy = torch.full((bs, prompt.shape[1] + gen_length), 0, dtype=torch.bfloat16).to(model.device)
                ...
                for i in range(steps_per_block):
                    ...
                    if hasattr(torch.cuda, "amp") and hasattr(torch.cuda.amp, "autocast"):
                        with torch.cuda.amp.autocast(enabled=self.args.fp16):
                            ...
                            entropy_t = self.token_entropy_from_logits(logits, temperature)
                            ...
                            del x0, confidence, transfer_index, entropy_t
    ...
    def _generate_and_score_completions(
        ...
        with unwrap_model_for_generation(self.model_wrapped, self.accelerator) as unwrapped_model:
            ...
            for i in range(0, prompt_ids.size(0), generation_batch_size):
                ...
                batch_prompt_completion_ids, entropy = self.generate(
                ...
                del batch_prompt_ids, batch_prompt_mask, batch_prompt_completion_ids, entropy
                torch.cuda.empty_cache()

            prompt_completion_ids = torch.cat(prompt_completion_ids_all, dim=0)
            entropy = torch.cat(entropy_all, dim=0)
        ...
        policy_entropy = (torch.sum(entropy * completion_mask) / torch.sum(completion_mask))
        ...
        policy_entropy = self.accelerator.gather_for_metrics(policy_entropy).float().mean().item()
        self._metrics[mode]["completion_length"].append(completion_length)
        self._metrics[mode]["zero_std_ratio"].append(zero_std_ratio)
        self._metrics[mode]["policy_entropy"].append(policy_entropy)
...
class d2StepMergeTrainer(GRPOTrainer):
    """
    Group Relative Policy Optimization (GRPO) Trainer for Diffusion Language Models.
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        completion_ids, completion_mask = inputs["completion_ids"], inputs["completion_mask"]
        mask_seeds = inputs["mask_seeds"]
        trajectory, trajectory_mask = inputs["trajectory"], inputs["trajectory_mask"]
        ...
        # Get the current iteration index and corresponding mask seed
        this_itr_idx = math.floor((self._step - 1) / self.args.gradient_accumulation_steps) % self.num_iterations
        this_traj_idx = (self._step - 1) % self.args.N
        
        this_itr_mask_seed = mask_seeds[this_itr_idx] # We don't apply mask for now, used as a placeholder
        this_itr_trajectory = trajectory[this_traj_idx][this_itr_idx].unsqueeze(0)
        # For now, all the trajectories within num_iterations share the same segmentation pattern, if using different patterns, need to add [this_itr_idx] back
        this_itr_trajectory_mask = trajectory_mask[this_traj_idx]
        # this_itr_trajectory_mask = trajectory_mask[this_traj_idx][this_itr_idx].unsqueeze(0)
        input_ids = input_ids.unsqueeze(0)
        per_token_logps = self._get_per_token_logps(model, input_ids, logits_to_keep, [this_itr_mask_seed], this_itr_trajectory)
        # Compute the KL divergence between the model and the reference model
        if self.beta != 0.0:
            ref_per_token_logps = inputs["ref_per_token_logps"][this_itr_idx].unsqueeze(0)
            per_token_kl = (
                torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1
            )
        ...
        advantages = inputs["advantages"]
        old_per_token_logps = (
            inputs["old_per_token_logps"][this_itr_idx].unsqueeze(0)
            if self.num_iterations > 1
            else per_token_logps.detach()
        )
        coef_1 = torch.exp(per_token_logps - old_per_token_logps)
        ...
        if self.beta != 0.0:
            per_token_loss = per_token_loss + self.beta * per_token_kl
        mask = (this_itr_trajectory_mask & completion_mask)
        loss = (per_token_loss * mask).sum() * self.args.N / completion_mask.sum()
        # Log the metrics
        # No need to log evaluation metrics for now
        # mode = "eval" if self.control.should_evaluate else "train"
    ...
    def generate(
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            # d2-StepMerge segments
            segments = [int(index * (steps / N)) for index in range(1, N)]
            trajectory = [x.clone()]
            total_steps = 0
            trajectory_mask = [torch.full((bs, prompt.shape[1] + gen_length), 0).to(model.device) for _ in range(N)]
                ...
                for i in range(steps_per_block):
                    ...
                    if hasattr(torch.cuda, "amp") and hasattr(torch.cuda.amp, "autocast"):
                        with torch.cuda.amp.autocast(enabled=self.args.fp16):
                            ...
                            x[transfer_index] = x0[transfer_index]
                            entropy[transfer_index] = entropy_t[transfer_index]
                            trajectory_mask[len(trajectory) - 1][transfer_index] = 1
                            del x0, confidence, transfer_index, entropy_t

                            # d2-StepMerge segments
                            total_steps += 1
                            if len(trajectory) < N and total_steps == segments[len(trajectory) - 1]:
                                trajectory.append(x.clone()) 

            return x, entropy[:, prompt.shape[1]:], trajectory, trajectory_mask
    ...
    def _get_per_token_logps(self, model, input_ids, logits_to_keep, mask_seeds, trajectory):
        """
        Calculate per-token log probabilities.
        """
        num_iterations, batch_size, seq_len = input_ids.size()
        ...
        traj = trajectory.reshape(num_iterations * batch_size, -1)
        model_input = torch.cat([perturbed_seq[:, :prompt_length], traj[:, prompt_length:]], dim=1)
        logits = self.get_logits(model, model_input, prompt_index, self.args.cfg_scale, self.args.mask_id)  # [num_iterations * batch_size, seq_len, vocab_size]
        completion_logits = logits[:, -logits_to_keep:, :]  
        completion_targets = input_ids.reshape(num_iterations * batch_size, -1)[:, -logits_to_keep:]
        flat_logits = completion_logits.reshape(-1, completion_logits.size(-1))
        flat_targets = completion_targets.reshape(-1)
        loss = F.cross_entropy(flat_logits, flat_targets, reduction="none")
        completion_log_probs = -loss.view(num_iterations * batch_size, logits_to_keep)
        per_token_logps = completion_log_probs.view(num_iterations, batch_size, logits_to_keep)
        per_token_logp = per_token_logps.to(torch.float32)

        return per_token_logp

    def _get_per_token_logps_oldandref(self, model, input_ids, logits_to_keep, mask_seeds, trajectory, trajectory_mask):
        """
        Calculate per-token log probabilities.
        """
        num_iterations, batch_size, seq_len = input_ids.size()
        ...
        per_token_logps = []
        for iter_idx in range(len(trajectory)):
            traj_i = trajectory[iter_idx].reshape(num_iterations * batch_size, -1)
            model_input_i = torch.cat([perturbed_seq[:, :prompt_length], traj_i[:, prompt_length:]], dim=1)
            traj_mask_i = trajectory_mask[iter_idx]
            logits_i = self.get_logits(model, model_input_i, prompt_index, self.args.cfg_scale, self.args.mask_id)  # [num_iterations * batch_size, seq_len, vocab_size]
            completion_logits_i = logits_i[:, -logits_to_keep:, :]  
            completion_targets = input_ids.reshape(num_iterations * batch_size, -1)[:, -logits_to_keep:]
            flat_logits_i = completion_logits_i.reshape(-1, completion_logits_i.size(-1))
            flat_targets = completion_targets.reshape(-1)
            loss_i = F.cross_entropy(flat_logits_i, flat_targets, reduction="none")
            completion_log_probs_i = -loss_i.view(num_iterations * batch_size, logits_to_keep)
            per_token_logps_i = completion_log_probs_i.view(num_iterations, batch_size, logits_to_keep)
            per_token_logps.append(per_token_logps_i * traj_mask_i)

        per_token_logp = torch.stack(per_token_logps).sum(dim=0)
        del per_token_logps, all_perturbed_seqs
        torch.cuda.empty_cache()
        per_token_logp = per_token_logp.to(torch.float32)
        return per_token_logp
    ...
    def _prepare_inputs(
        ...
        if mode == "train":
            if self.state.global_step % self.num_iterations == 0:
                if self._step % self.args.N == 0:    
                    inputs = self._generate_and_score_completions(inputs)
                    self._buffered_inputs[int(self._step / self.args.N) % int(self.args.gradient_accumulation_steps / self.args.N)] = inputs
                else:
                    inputs = self._buffered_inputs[math.floor(self._step / self.args.N) % int(self.args.gradient_accumulation_steps / self.args.N)]
            else:
                inputs = self._buffered_inputs[math.floor(self._step / self.args.N) % int(self.args.gradient_accumulation_steps / self.args.N)]
            self._step += 1
        else:
    ...
    def _generate_and_score_completions(
        ...
        with unwrap_model_for_generation(self.model_wrapped, self.accelerator) as unwrapped_model:
            ...
            for i in range(0, prompt_ids.size(0), generation_batch_size):
                ...
                # unintended attention to padding tokens when num_generations is smaller.
                # As currently we find Llada's modeling file does not handle attention mask. We will address this in future update soon.
                batch_prompt_completion_ids, entropy, trajectory, trajectory_mask = self.generate(
                    model=unwrapped_model,
                    prompt=batch_prompt_ids,
                    steps=steps,
                    gen_length=gen_length,
        ...
        with torch.no_grad():
            if self.num_iterations > 1:
                ...
                trajectory_expanded = [traj.unsqueeze(0).expand(self.num_iterations, -1, -1) for traj in trajectory]
                trajectory_mask_expanded = [traj_mask[:, -logits_to_keep:].unsqueeze(0) for traj_mask in trajectory_mask]
                old_per_token_logps = self._get_per_token_logps_oldandref(
                    self.model, prompt_completion_ids_expanded, logits_to_keep, mask_seeds, trajectory_expanded, trajectory_mask_expanded
                )
                all_old_per_token_logps = old_per_token_logps
            else:
                old_per_token_logps = None
            ...
            if self.beta == 0.0:
                ...
                with self.accelerator.unwrap_model(self.model).disable_adapter():
                    ref_per_token_logps = self._get_per_token_logps_oldandref(
                        self.model, prompt_completion_ids_expanded, logits_to_keep, mask_seeds, trajectory_expanded, trajectory_mask_expanded
                    )
                    all_ref_per_token_logps = ref_per_token_logps
        ...
        return {
            ...
            "advantages": advantages,
            "mask_seeds": mask_seeds,  # Store all mask seeds for consistent mask patterns
            "trajectory": trajectory_expanded,
            "trajectory_mask": trajectory_mask_expanded
        }